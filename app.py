...
for i, (sym, (name, kind)) in enumerate(filtered.items()):
    ...
    progress.progress((i + 1) / total_assets, text=f"Scanning {sym}…")

progress.empty()

# ── filter and sort ──────────────────────────────────────────────────────────
results = {sym: d for sym, d in results.items() if d["primary"]["score"] >= min_score}
...
# ── Signal Cards ───────────────────────────────────────────────────────────────
with tab_cards:
    st.markdown(f'<div class="section-head">Primary TF: {primary_tf} • Sorted by Alpha Score</div>', unsafe_allow_html=True)
    cols = st.columns(4)
    for idx, (sym, data) in enumerate(sorted_results):
        ...
        tf_badges = "".join(
            ...
            f'padding:2px 6px;border-radius:3px;display:inline-block;'
...
# ── Full Table ─────────────────────────────────────────────────────────────────
with tab_table:
    st.markdown('<div class="section-head">All Assets • All Timeframes</div>', unsafe_allow_html=True)
    rows = []
    for sym, data in sorted_results:
        ...
        # RSI FIX: Pull from metrics['RSI']
        row["RSI"] = p["metrics"].get("RSI", "N/A")
        rows.append(row)
...
# ── Deep Dive ──────────────────────────────────────────────────────────────────
with tab_detail:
    st.markdown('<div class="section-head">Per-Indicator Breakdown</div>', unsafe_allow_html=True)
    if sorted_results:
        selected_sym = st.selectbox("Select asset", [sym.replace("-USD", "") for sym, _ in sorted_results])
